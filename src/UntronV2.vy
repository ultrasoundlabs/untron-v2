...
# Untron V2: Optimistic Tron<->EVM USDT bridge
# 
...
interface IProver:
    def proveClaim(receiver: bytes20, amount: uint256, timestamp: uint256, period: uint256, proof: Bytes[4096]) -> bool: view
...
def proveClaim(receiver: bytes20, atAmount: uint256, proof: Bytes[4096]):
    ...
    assert staticcall self.prover.proveClaim(
